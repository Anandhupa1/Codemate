...
boxes.forEach((box) => {
  box.addEventListener("click", () => {
    boxes.forEach((box) => {
      box.style.backgroundColor = "#5184eb";
...
function createDom(data, status) {
  ...
  if (!trainer.user.approved) {
    thead.innerHTML =
      "<h2>Your Request is still Pending, please wait to be Approved.</h2>";
    return;
  }
  ...
  if (status == "pending") {
    data?.bookings?.forEach((el, i) => {
      ...
      tr.innerHTML = `<td>${i + 1}</td>
            ...
            <td onclick="sendResponse('${
              el._id
            }','accepted')" class="accept"><div>Accept</div></td>
            <td onclick="sendResponse('${
              el._id
            }','rejected')" class="reject"><div>Reject</div></td>`;
      tbody.append(tr);
    });
  } else if (status == "accepted") {
    data?.bookings?.forEach((el, i) => {
      ...
      tr.innerHTML = `<td>${i + 1}</td>
            ...
            <td class="start">${formatTime(el.start_time)}</td>
            <td class="end">${formatTime(el.end_time)}</td>
            <td onclick="meet('${el._id}','${
        el.client.name
      }')" class="meet"><div>Invite</div></td>`;
      tbody.append(tr);
    });
  } else {
